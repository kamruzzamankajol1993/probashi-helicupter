...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Client Say  List</h4>
...
<div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="float-right d-md-block">
                                <div class="dropdown">
                                @if (Auth::guard('admin')->user()->can('clientSayAdd'))

                                <a class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light"  href="{{ route('admin.client_say_part.create') }}">
...
<td>{!! $user->status !!}</td>
                                    ...
                                    <td>
                                        ...
                                        @if (Auth::guard('admin')->user()->can('clientSayView'))
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"> Client Say Information</h5>
      ...
      <div class="modal-body">
        ...
        {!! $user->des !!}
                                        ...
                                        @endif
                                      @if (Auth::guard('admin')->user()->can('clientSayUpdate'))



                                          <a class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light"  href="{{ route('admin.client_say_part.edit',$user->id) }}">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Client Say  Information</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
        </button>
      </div>
      <div class="modal-body">
        <form class="custom-validation" action="{{ route('admin.client_say_part.update') }}" method="post" enctype="multipart/form-data">
                              @csrf
                                <div class="row">
...
@endif
                                  ...
                                  @if (Auth::guard('admin')->user()->can('clientSayDelete'))
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.client_say_part.delete',$user->id) }}" method="POST" style="display: none;">
...
<div class="modal fade bs-example-modal-lg456" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add Client Say  Information</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <div class="modal-body">
                <form class="custom-validation" action="{{ route('admin.client_say_part.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                       <div class="row">
