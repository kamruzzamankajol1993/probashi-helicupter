...
@section('title')
About Us Service List| {{ $ins_name }}
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">About Us Service List</h4>
...
<th>title</th>
                                        ...
                                        <tbody>
                                            @foreach ($about_us_services_list as $user)
...
<td>{!! $user->des !!}</td>
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update About Us Service Information</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
        </button>
      </div>
      <div class="modal-body">
        <form class="custom-validation" action="{{ route('admin.about_us_services.update') }}" method="post" enctype="multipart/form-data">
                              @csrf
                                <div class="row">
...
<div class="row">
                        ...
                        <input type="hidden" class="form-control form-control-sm" value="{{ $user->id }}" name="id" placeholder="Enter Name">
                            ...
                            <div class="form-group col-md-12 col-sm-12">
                                ...
                                <input type="text" class="form-control form-control-sm" value="{{ $user->image }}" id="name" name="image" placeholder="Enter Icon">
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.about_us_services.delete',$user->id) }}" method="POST" style="display: none;">
...
<div class="modal fade bs-example-modal-lg456" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add About Us Service Information</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <div class="modal-body">
                <form class="custom-validation" action="{{ route('admin.about_us_services.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                       <div class="row">
                  ...
                  <div class="form-group col-md-12 col-sm-12">
                      <label for="name">Icon</label>
                      <input type="text" class="form-control form-control-sm" id="name" name="image" placeholder="Enter Icon">
